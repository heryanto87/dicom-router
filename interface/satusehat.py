...
def imagingstudy_post(filename, id):
    ...
    if id == None:
        print(headers)
        print(config.url+config.fhir_pathsuffix + "/ImagingStudy")
...
def dicom_push(assocId, study_iuid, imagingStudyID):
    if imagingStudyID is None: return None
    ...
    for n in range(len(instances)):
        ...
        try:
            payload = open(filename, 'rb')
            str = ""
            print("IMAGING STUDY URL:", config.url+config.dicom_pathsuffix)
...
def get_dcm_config(token):
    ...
    path = config.fhir_pathsuffix+"/dcm_cfg"
    res = requests.get(url=config.url+path, headers=headers)
    data = res.json()
    return data
